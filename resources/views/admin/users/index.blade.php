...
@section('title')
{{-- master title here --}}
{{ trans('main_translate.Users List') }}
@endsection
...
{{ trans('main_translate.Users List') }}
    @can('create_users')
        <a class="btn btn-primary btn-sm" href="{{ route('create_user') }}">{{ trans('main_translate.Add User') }}</a>
    @endcan
@endsection
            ...
            <table id="example1" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        ...
                        <th>{{ trans('main_translate.Reseller') }}</th>
                ...
                <tr>
                        ...
                        <td>
                            @if (is_array($user->reseller))
                            @foreach ($user->reseller as $id => $name)
                            <span class="badge badge-success">{{$name}}</span>
                            @endforeach
                            @else
                            <span class="badge badge-success"></span>
                        ...
                        <td>
                            @if (is_array($user->pay_method))
                            @foreach ($user->pay_method as $id => $name)
                            <span class="badge badge-success">{{$name}}</span>
                            @endforeach
                            @else
                            <span class="badge badge-success">{{ $user->pay_method }}</span>
                            @endif
                        </td>
                        <td>{{ $user->notes }}</td>
